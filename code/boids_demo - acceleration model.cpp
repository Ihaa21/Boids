...
DEMO_INIT(Init)
{
    ...
    // NOTE: Init Boids
    {
        ...
        DemoState->MaxSpeed = 3.0f;
        DemoState->MaxSteerSpeed = 2.0f;

        DemoState->BoidRadiusSq = Square(0.4f);
        DemoState->AvoidRadiusSq=  Square(0.2f);
        DemoState->TerrainAvoidRadius = 0.15;

        DemoState->TerrainRadius = 2.0f;
        
        DemoState->AvoidTerrainWeight = 5.0f;
        DemoState->AvoidBirdWeight = 1.0f;
        DemoState->AlignFlockWeight = 1.0f;
        DemoState->MoveToFlockWeight = 1.0f;
                
        DemoState->NumBoids = 50;
        DemoState->BoidPositions = PushArray(&DemoState->Arena, v3, DemoState->NumBoids);
        ...
        for (u32 BoidId = 0; BoidId < DemoState->NumBoids; ++BoidId)
        {
            ...
            DemoState->BoidPositions[BoidId] = V3(0, 0, 0);
...
inline v3 SteerTowards(v3 Velocity, v3 NewVec)
{
    v3 Result = NormalizeSafe(NewVec) * DemoState->MaxSpeed - Velocity;

    // NOTE: Clamp our velocity
    f32 ResultLength = Length(Result);
    if (ResultLength > DemoState->MaxSteerSpeed)
    {
        Result = (DemoState->MaxSteerSpeed / ResultLength) * Result;
    }
...
DEMO_MAIN_LOOP(MainLoop)
{
    ...
    // NOTE: Update Ui State
    {
                ...
                
        {
            ...
            UiPanelHorizontalSlider(&Panel, 0.0f, 5.0f, &DemoState->AvoidTerrainWeight);
            UiPanelNumberBox(&Panel, 0.0f, 5.0f, &DemoState->AvoidTerrainWeight);
            UiPanelNextRow(&Panel);
            
            UiPanelNextRowIndent(&Panel);
            UiPanelText(&Panel, "Avoid Bird Weight:");
            UiPanelHorizontalSlider(&Panel, 0.0f, 5.0f, &DemoState->AvoidBirdWeight);
            UiPanelNumberBox(&Panel, 0.0f, 5.0f, &DemoState->AvoidBirdWeight);
            UiPanelNextRow(&Panel);
            
            UiPanelNextRowIndent(&Panel);
            UiPanelText(&Panel, "Align Flock Weight:");
            UiPanelHorizontalSlider(&Panel, 0.0f, 5.0f, &DemoState->AlignFlockWeight);
            UiPanelNumberBox(&Panel, 0.0f, 5.0f, &DemoState->AlignFlockWeight);
            UiPanelNextRow(&Panel);
            
            UiPanelNextRowIndent(&Panel);
            UiPanelText(&Panel, "Move To Flock Weight:");
            UiPanelHorizontalSlider(&Panel, 0.0f, 5.0f, &DemoState->MoveToFlockWeight);
            UiPanelNumberBox(&Panel, 0.0f, 5.0f, &DemoState->MoveToFlockWeight);
            UiPanelNextRow(&Panel);
    ...
    // NOTE: Upload scene data
    {
        ...
        // NOTE: Populate scene
        {
            // NOTE: Add Instances
            {
                ...
                for (u32 BoidId = 0; BoidId < DemoState->NumBoids; ++BoidId)
                {
                    v3 NewBoidPosition = DemoState->BoidPositions[BoidId];
                    v3 NewBoidVelocity = DemoState->BoidVelocities[BoidId];
                    v3 NewBoidAccel = {};

                    b32 FoundAvoid = false;
                    ...
                    for (u32 NearbyBoidId = 0; NearbyBoidId < DemoState->NumBoids; ++NearbyBoidId)
                    {
                        if (NearbyBoidId != BoidId)
                        {
                            ...
                            if (DistanceSq < DemoState->AvoidRadiusSq)
                            {
                                // NOTE: Avoidance
                                FoundAvoid = true;
                                AvgFlockAvoidance -= DistanceVec; // * 1.0f / DistanceSq;
                            }
                        }
                    }
                    ...
                    {
                        NewBoidAccel += DemoState->MoveToFlockWeight * SteerTowards(NewBoidVelocity, NormalizeSafe(AvgFlockPos - NewBoidPosition));
                        NewBoidAccel += DemoState->AlignFlockWeight * SteerTowards(NewBoidVelocity, AvgFlockDir);
                        if (FoundAvoid)
                        {
                            NewBoidAccel += DemoState->AvoidBirdWeight * SteerTowards(NewBoidVelocity, AvgFlockAvoidance);
                        }
                    }

                    if (CloseToWall)
                    {
                        NewBoidAccel += DemoState->AvoidTerrainWeight * SteerTowards(NewBoidVelocity, AvoidWallDir);
                    }
                    
                    // NOTE: Apply Acceleration, clamp velocity
                    NewBoidVelocity += NewBoidAccel * FrameTime;
